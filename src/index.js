export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/metrics") {
      ...
      // 1) Revenue via GHL Payments (easy win)
      let revenue = 0;
      let txCount = 0;
      ...
      try {
        ...
        // Most commonly cents -> dollars
        revenue = revenueRaw / 100;
      } catch (e) {
        // We still return a valid JSON shape even if payments fails
        // (so the UI doesn't crash)
      }

      // Return the exact JSON shape your UI expects
      return json({
        totalLeads: 0,          // next step
        appointments: 0,        // next step
        showRate: 0,            // next step
        revenue: revenue,

        leadSources: [
          // Example placeholders (replace next step)
          // { label: "Organic/Direct", count: 0 },
        ],
        apptTypes: [],
        topAds: [],
        ...
        revenueMetrics: {
          ...
          avgTransaction: txCount ? revenue / txCount : 0,
...
async function ghlGet(url, env) {
  ...
  if (!res.ok) throw new Error(`GHL error ${res.status}: ${text}`);
  return data;
}
...
// ----------------------------
// FULL DASHBOARD UI (your code)
// Uses Worker endpoint: /metrics
// ----------------------------
...
(() => {
  // ---------------------------------------------------------
  // IMPORTANT CHANGE:
  // We always pull from THIS Worker so GHL doesn't strip params.
  // ---------------------------------------------------------
  const apiUrl = window.location.origin + "/metrics";
  ...
  function percentOf(part, total){
    if(!total) return 0;
    return clampPct((Number(part)||0)/(Number(total)||0)*100);
  }

  async function loadData(rangeKey){
    try{
      const res = await fetch(\`\${apiUrl}?range=\${encodeURIComponent(rangeKey)}\`, { credentials: "omit" });
      ...
      <div class="dash">
        ...
        <div class="grid2">
          <div class="card">
            ...
            \${leadSources.length ? leadSources.slice(0,8).map(ls =>
              progressRow(ls.label, ls.count, percentOf(ls.count, totalLeadSourceCount))
            ).join("") : emptyState("No lead source data yet.")}
          </div>

          <div class="card">
            <p class="sectionTitle">Appointment Types</p>
            <p class="sectionSub">Distribution of appointments by pipeline/type</p>
            \${apptTypes.length ? apptTypes.slice(0,8).map(a =>
              progressRow(a.label, \`\${a.count} (\${percentOf(a.count,totalApptTypeCount).toFixed(1)}%)\`, percentOf(a.count,totalApptTypeCount))
            ).join("") : emptyState("No appointment type data yet.")}
          </div>
        </div>
